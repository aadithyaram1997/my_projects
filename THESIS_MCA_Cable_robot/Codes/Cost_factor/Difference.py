...
class ClassicalWashout:
    ...
    def process(self, accel_data, rotational_data):
        ...
        # forcex = accel_data['x'] * Pilot_mass
        # forcey = accel_data['y'] * Pilot_mass
        # forcez = (accel_data['z'] + Acc_g) * Pilot_mass
       ...
       # Integrate HP accelerations to obtain velocities and then positions
        ...
        # yaw_angle = it.cumulative_trapezoid(washout_yaw, dx=1 / self.fs, initial=0)
...
if __name__ == "__main__":
    ...
    force_x_factor = (force_x_post) - (force_x)
    force_y_factor = (force_y_post) - (force_y)
    force_z_factor = (force_z_post) - (force_z)
...
#     plt.title("Processed Angle Data")
    ...
    plt.boxplot([force_x_factor, force_y_factor, force_z_factor],
               labels=['Force X Factor', 'Force Y Factor', 'Force Z Factor'],
               showfliers=True)  # showfliers=True includes outliers in the plot
    plt.title("Box Plot of Force Factors")
    plt.ylabel("Force Factor")
    plt.xlabel("Force Components")
