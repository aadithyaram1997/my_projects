...
class ClassicalWashout:
    ...
    def process(self, accel_data, rotational_data):
        ...
        #Damping for washout
        #for washout before position


       # For washout after position
       # Integrate HP accelerations to obtain velocities and then positions
...
def cost_z(damping_z, washout_algorithm, accel_data, time):
    ...
    num_elements = len(force_z_factor)
    print(f"Number of elements in force_z_factor: {num_elements}")
    print(f"washout_damping_z: {damping_z[0]}, Average Force_X_Factor: {average_force_z_factor}")
...
if __name__ == "__main__":
    ...
    output_df.to_csv(output_csv_file, index=False)
     ...
     # Calculate forces based on pilot mass and save to CSV
 #   force_x = (accel_data['x'] * Pilot_mass)
    force_y = (accel_data['y'] * Pilot_mass)
    force_z = (accel_data['z'] + Acc_g) * Pilot_mass
