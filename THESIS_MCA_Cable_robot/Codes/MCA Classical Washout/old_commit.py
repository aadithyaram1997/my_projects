...
class ClassicalWashout:
    ...
    def process(self, accel_data, rotational_data):
       ...
       # Integrate HP accelerations to obtain velocities and then positions
        ...
        v_x = np.gradient(pos_x, 1 / self.fs)
        v_y = np.gradient(pos_y, 1 / self.fs)
        v_z = np.gradient(pos_z, 1 / self.fs)
        ...
        lp_acc_x_g = lowpass_accel_x * (-Acc_g)
        lp_acc_y_g = lowpass_accel_y * (-Acc_g)
        ...
        tilt_roll_rate = np.gradient(tilt_x_angle_deg, 1 / self.fs)
        tilt_pitch_rate = np.gradient(tilt_y_angle_deg, 1 / self.fs)
       ...
       # print(washout_yaw)
        ...
        return {
            ...
            'lp_acc_x_g' : lp_acc_x_g,
            'lp_acc_y_g' : lp_acc_y_g,
            ...
            'tilt_roll_rate' : tilt_roll_rate,
            'tilt_pitch_rate' : tilt_pitch_rate,
...
if __name__ == "__main__":
    # Load the CSV file
    csv_file = "C:\\Users\\mrf-ar\\motioncueing\\Flight gear paramenters\\mrf_the_epic_swiss_run.csv"  # Replace with your file path
    ...
    output_csv_file = "processed_motion_data_mrf_swiss.csv"
  ...
  #   Plot raw and processed data
    ...
    plt.ylabel("Position (m)")
    plt.legend()
    plt.title(" Processed Position without washout")
    ...
    plt.subplot(426)
    plt.plot(data['Time'], processed_movement['v_x'], label="x")
    plt.plot(data['Time'], processed_movement['v_y'], label="y")
    plt.plot(data['Time'], processed_movement['v_z'], label="z")
    plt.xlabel("Time (s)")
    plt.ylabel("Velocity (m/s)")
    plt.legend()
    plt.title("Velocity after washout")
    ...
    plt.ylabel("Position (m)")
    plt.legend()
    plt.title("Processed Position with washout")
   ...
   # plt.plot(data['Time'], accel_data['y'], label="Y")
    ...
    plt.ylabel("Acceleration (m/s²)")
    plt.legend()
    plt.title("HP Accelerations")

    plt.subplot(413)
    plt.plot(data['Time'], processed_movement['lp_acc_x_g'], label="X")
   # plt.plot(data['Time'], processed_movement['highpass_accel_y_2'], label="Y")
    #plt.plot(data['Time'], processed_movement['highpass_accel_z_2'], label="Z")
    plt.xlabel("Time (s)")
    plt.ylabel("Acceleration (m/s²)")
    plt.legend()
    plt.title("LP Accelerations")
   ...
   # plt.plot(data['Time'], accel_data['y'], label="Y")
    ...
    plt.subplot(413)
    plt.plot(data['Time'], processed_movement['lp_acc_y_g'], label="Y")
   # plt.plot(data['Time'], processed_movement['highpass_accel_y_2'], label="Y")
    #plt.plot(data['Time'], processed_movement['highpass_accel_z_2'], label="Z")
    plt.xlabel("Time (s)")
    plt.ylabel("Acceleration (m/s²)")
    ...
    plt.subplot(526)
    plt.plot(data['Time'],  processed_movement['tilt_pitch_rate'], label="tilt roll rate")
    plt.plot(data['Time'],  processed_movement['tilt_roll_rate'], label="tilt pitch rate")
    ...
    plt.ylabel("Rate (deg/s)")
    plt.legend()
    plt.title("Tilt rates")


    plt.subplot(527)
    plt.plot(data['Time'], processed_movement['tilt_y_angle_deg'], label="Tilt roll angle")
    plt.plot(data['Time'], processed_movement['tilt_x_angle_deg'], label="Tilt pitch angle")
   ...
   # plt.plot(data['Time'], processed_movement['yaw'], label="yaw")
    ...
    plt.ylabel("Angle (deg)")
    plt.legend()
    plt.title("Rotational angles without tilt")
