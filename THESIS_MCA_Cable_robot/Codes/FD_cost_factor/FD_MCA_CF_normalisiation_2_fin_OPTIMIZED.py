...
def cost_force_factor(damping_value, axis, washout_algorithm, accel_data, rotational_data, time, mass, acc_g):
    ...
    if axis == 'x':
        ...
        accel = np.gradient(vel, time)
        final_accel = accel + (-1 * processed['lowpass_accel_x'] * acc_g)
    elif axis == 'y':
        raw_force = accel_data['y'] * mass
        vel = np.gradient(processed['y'], time)
        accel = np.gradient(vel, time)
        final_accel = accel + (-1 * processed['lowpass_accel_y'] * acc_g)
    elif axis == 'z':
        raw_force = (accel_data['z'] + acc_g) * mass
        vel = np.gradient(processed['z'], time)
        accel = np.gradient(vel, time)
        final_accel = accel + acc_g

    post_force = final_accel * mass
    denom = np.abs(raw_force) + 3
    force_factor = np.abs(post_force - raw_force) / denom
    force_factor = np.nan_to_num(force_factor)
    return np.mean(force_factor)


def force_aware_objective(damping_value, axis, washout_algorithm, accel_data, rotational_data, time, mass, acc_g, Irobot):
    setattr(washout_algorithm, f"washout_damping_{axis}", damping_value[0])
    processed = washout_algorithm.process(accel_data, rotational_data)
    ...
    return cost_force_factor(damping_value, axis, washout_algorithm, accel_data, rotational_data, time, mass, acc_g)


def optimize_damping(washout_algorithm, accel_data, rotational_data, time, Irobot, mass, acc_g):
    print("\n--- Damping Optimization Started ---")

    def run_opt(axis, initial_value, bounds):
        result = minimize(
            force_aware_objective,
            [initial_value],
            args=(axis, washout_algorithm, accel_data, rotational_data, time, mass, acc_g, Irobot),
            bounds=[bounds],
            method='L-BFGS-B',
            options={'maxiter': 100}
        )
    ...
    for axis, bounds in zip(['x', 'y', 'z'], [(0.0005, 0.02), (0.0005, 0.02), (0.0005, 0.02)]):
        initial = getattr(washout_algorithm, f'washout_damping_{axis}')
        result = run_opt(axis, initial, bounds)

        print(f"{axis.upper()} → tried damping: {initial:.5f}, optimized: {result.x[0]:.5f}, cost: {result.fun:.5f}")

        if result.success and result.fun < 1e5:
            setattr(washout_algorithm, f"washout_damping_{axis}", result.x[0])
            print(f"✔ Optimized washout_damping_{axis}: {result.x[0]}")
        else:
            print(f"✖ Optimization failed for washout_damping_{axis}. Keeping previous value.")
