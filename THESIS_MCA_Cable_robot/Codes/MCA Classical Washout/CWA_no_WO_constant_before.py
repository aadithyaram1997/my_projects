...
class ClassicalWashout:
    ...
    def process(self, accel_data, rotational_data):
        ...
        washout_x = highpass_accel_x_2 * np.exp(-np.linspace(0, 0, len(highpass_accel_x_2)))
        washout_y = highpass_accel_y_2 * np.exp(-np.linspace(0, 0, len(highpass_accel_y_2)))
        washout_z = highpass_accel_z_2 * np.exp(-np.linspace(0, 0, len(highpass_accel_z_2)))
        ...
        # pos_y = washout_x * np.exp(-self.washout_damping_x * np.arange(len(washout_x)) / self.fs)
        # pos_z = washout_x * np.exp(-self.washout_damping_x * np.arange(len(washout_x)) / self.fs)
        ...
        #washout before angles

        washout_roll = highpass_roll_2 * np.exp(-np.linspace(0, 0, len(highpass_roll_2)))
        washout_pitch = highpass_pitch_2 * np.exp(-np.linspace(0, 0, len(highpass_pitch_2)))
        washout_yaw = highpass_yaw_2 * np.exp(-np.linspace(3, 5, len(highpass_yaw_2)))
