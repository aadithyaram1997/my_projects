...
def ForceDis(Irobot, x, y, z, roll, pitch, yaw):
    try:
        return Irobot.getForceDistribution(x, y, z, roll, pitch, yaw)
    except Exception as e:
        print(f"ForceDis failed at ({x:.3f}, {y:.3f}, {z:.3f}, {roll:.3f}, {pitch:.3f}, {yaw:.3f}): {e}")
        return None
...
def main():
    ...
    csv_file = "C:\\Users\\mrf-ar\\motioncueing\\Flight gear paramenters\\mrf_the_epic_swiss_run.csv"
    ...
    washout_damping_z = 0.005
    washout_damping_a = 0.005
    washout_damping_b = 0.007
    washout_damping_c = 0.018
   ...
   # Small epsilon to prevent division by zero
    ...
    force_y_factor = np.abs(force_y_post - force_y) / (np.abs(force_y) + 5)
    force_z_factor = np.abs(force_z_post - force_z) / (np.abs(force_z) + 2)
...
#     plt.title("Processed Angle Data")
    ...
    plt.plot(data['Time'], force_factor['Force_Y_Factor'], label="Force_Y_Factor")
...
def is_force_feasible(Irobot, positions):
    for pose in positions:
        force_distribution = ForceDis(Irobot, *pose)
        if not isinstance(force_distribution, (list, tuple, np.ndarray)):
            return False
        if all(f == 0 for f in force_distribution):
            return False
    return True

def extract_positions(movement_dict):
    return list(zip(
        movement_dict['x'], movement_dict['y'], movement_dict['z'],
        movement_dict['roll'], movement_dict['pitch'], movement_dict['yaw']
    ))

def optimize_washout_damping():
    from WiPy3 import Irobot, Ikin, Iws
    setGeometry(Irobot, Ikin, Iws)
    calcWorkspaceforce(Iws)
    ...
    fs = 10
    hp_acc_x_1 = hp_acc_y_1 = hp_acc_z_1 = 0.05
    hp_acc_x_2 = hp_acc_y_2 = hp_acc_z_2 = 0.05
    lp_acc_x = lp_acc_y = lp_acc_z = 0.05
    hp_roll_1 = hp_pitch_1 = hp_yaw_1 = 0.00001
    hp_roll_2 = hp_pitch_2 = hp_yaw_2 = 0.00001
    scale_x_hp = scale_y_hp = scale_z_hp = 0.8
    scale_x_lp = scale_y_lp = 0.8
    scale_a_hp = scale_b_hp = 0.8
    scale_c_hp = 0.65
    ...
    def cost_fn(params):
        nonlocal best_feasible_cost, best_feasible_params
        wx, wy, wz = params
        ...
        if not is_force_feasible(Irobot, poses):
            return 1e6  # Penalty for infeasible

        # Calculate force factors
        v_x = np.gradient(result["x"], data["Time"])
        v_y = np.gradient(result["y"], data["Time"])
        v_z = np.gradient(result["z"], data["Time"])
        ...
        final_accel_y = accel_y_post + (-1 * result["lowpass_accel_y"] * Acc_g)
        final_accel_z = accel_z_post + Acc_g
        ...
        fx = np.abs(force_x_post - force_x) / (np.abs(force_x) + 5)
        fy = np.abs(force_y_post - force_y) / (np.abs(force_y) + 5)
        fz = np.abs(force_z_post - force_z) / (np.abs(force_z) + 2)

        cost = np.nanmean(fx) + np.nanmean(fy) + np.nanmean(fz)

        # Save best feasible params
        if cost < best_feasible_cost:
            best_feasible_cost = cost
            best_feasible_params = params
            print(f"✅ Feasible | Damping: {params} | Cost: {cost:.4f}")

        return cost

    initial = [0.01, 0.05, 0.05]
    bounds = [(0.00000001, 0.01), (0.00000001, 0.01), (0.0000001, 0.01)]

    print("🔁 Running optimization...\n")
    result = minimize(cost_fn, initial, bounds=bounds, method="L-BFGS-B")

    print("\n--- Optimization Finished ---")
    print(f"Reported result (may be infeasible): {result.x}, cost: {result.fun}")
    print(f"✅ Best feasible result: {best_feasible_params}, cost: {best_feasible_cost}")

    return best_feasible_params

if __name__ == "__main__":
    optimized = optimize_washout_damping()
    print("Optimized washout damping x, y, z:", optimized)
