...
def cost_function(params, accel_data, rotational_data, Irobot):
    ...
    force_distribution_valid = ForceDis(Irobot, processed_pose['x'], processed_pose['y'], processed_pose['z'],
                                        processed_pose['roll'], processed_pose['pitch'], processed_pose['yaw'])
    
    penalty = 0
    if not force_distribution_valid:
        penalty = 1e6  # Large penalty for invalid force distributions

    # You might also want to add a small cost component based on the processed poses to ensure smooth optimization
    pose_cost = np.sum(np.square(processed_pose['x'])) + np.sum(np.square(processed_pose['y'])) + np.sum(np.square(processed_pose['z']))
    
    return penalty + pose_cost
