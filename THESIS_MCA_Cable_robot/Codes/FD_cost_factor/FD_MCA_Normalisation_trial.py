...
def cost_x(damping_x, washout_algorithm, accel_data, time):
    ...
    force_x_factor = np.abs(force_x_post - force_x) / (np.abs(force_x) + 3)
    force_x_factor = np.where(np.isfinite(force_x_factor), force_x_factor, 0)  # Replace infinities with 0
    force_x_factor = np.nan_to_num(force_x_factor)  # Replace NaN values with 0
  ...
  # Store force values globally
    force_x_storage['time'] = time
    force_x_storage['force_x'] = force_x
    force_x_storage['force_x_post'] = force_x_post
    force_x_storage['force_x_factor'] = force_x_factor
    print(f"Number of elements in force_x_factor: {num_elements}")
    print(f"washout_damping_x: {damping_x[0]}, Average Force_X_Factor: {average_force_x_factor}")
...
def cost_y(damping_y, washout_algorithm, accel_data, time):
    ...
    v_y = np.gradient(processed_movement['y'], data['Time'])
    accel_y_post = np.gradient(v_y, data['Time'])
    final_accel_y = accel_y_post + (-1*(processed_movement['lowpass_accel_y']*Acc_g))
    force_y_post = (final_accel_y * Pilot_mass)
    force_y_factor = np.abs(force_y_post - force_y) / (np.abs(force_y) + 2)
    force_y_factor = np.where(np.isfinite(force_y_factor), force_y_factor, 0)  # Replace infinities with 0
    force_y_factor = np.nan_to_num(force_y_factor)  # Replace NaN values with 0

    average_force_y_factor = np.mean(force_y_factor)
    num_elements = len(force_y_factor)
# Store force values globally
    force_y_storage['time'] = time
    force_y_storage['force_y'] = force_y
    force_y_storage['force_y_post'] = force_y_post
    force_y_storage['force_y_factor'] = force_y_factor
    print(f"Number of elements in force_y_factor: {num_elements}")
    print(f"washout_damping_y: {damping_y[0]}, Average Force_Y_Factor: {average_force_y_factor}")
...
def cost_z(damping_z, washout_algorithm, accel_data, time):
    ...
    v_z = np.gradient(processed_movement['z'], data['Time'])
    accel_z_post = np.gradient(v_z, data['Time'])
    final_accel_z = accel_z_post + Acc_g
    force_z_post = (final_accel_z * Pilot_mass)
    force_z_factor = np.abs(force_z_post - force_z) / (np.abs(force_z) + 10)
    force_z_factor = np.where(np.isfinite(force_z_factor), force_z_factor, 0)  # Replace infinities with 0
    force_z_factor = np.nan_to_num(force_z_factor)  # Replace NaN values with 0

    average_force_z_factor = np.mean(force_z_factor)
    num_elements = len(force_z_factor)# Store force values globally
    force_z_storage['time'] = time
    force_z_storage['force_z'] = force_z
    force_z_storage['force_z_post'] = force_z_post
    force_z_storage['force_z_factor'] = force_z_factor

    print(f"Number of elements in force_z_factor: {num_elements}")
    print(f"washout_damping_z: {damping_z[0]}, Average Force_Z_Factor: {average_force_z_factor}")
...
#     result_x = minimize(cost_x, [washout_algorithm.washout_damping_x], args=(washout_algorithm, accel_data, time), bounds=[(0, 0.1)])
...
#     result_y = minimize(cost_y, [washout_algorithm.washout_damping_y], args=(washout_algorithm, accel_data, time), bounds=[(0.000001, 0.01)])
...
#     result_z = minimize(cost_z, [washout_algorithm.washout_damping_z], args=(washout_algorithm, accel_data, time), bounds=[(0.000001, 0.01)])
...
def optimize_damping(washout_algorithm, accel_data, time, Irobot):
    def validate_damping(damping_values, axis):
        """Checks if force distribution is valid for given damping values"""
        if axis == 'x':
            washout_algorithm.washout_damping_x = damping_values[0]
        elif axis == 'y':
            washout_algorithm.washout_damping_y = damping_values[0]
        elif axis == 'z':
            washout_algorithm.washout_damping_z = damping_values[0]

        processed_movement = washout_algorithm.process(accel_data, rotational_data)
        force_distribution_valid = False
        ...
        for i in range(len(time)):
            ...
            if force_distribution and not all(f == 0 for f in force_distribution):  # Ensures values are non-zero
                force_distribution_valid = True
                break  # Exit loop if at least one valid force distribution is found

        if not force_distribution_valid:
            return np.inf  # Return large penalty for invalid damping

        return cost_x(damping_values, washout_algorithm, accel_data, time) if axis == 'x' else \
               cost_y(damping_values, washout_algorithm, accel_data, time) if axis == 'y' else \
               cost_z(damping_values, washout_algorithm, accel_data, time)

    # Optimize washout_damping_x
    result_x = minimize(validate_damping, [washout_algorithm.washout_damping_x], args=('x',), bounds=[(0, 0.1)])
    optimized_damping_x = result_x.x[0] if result_x.fun != np.inf else washout_algorithm.washout_damping_x

    # Optimize washout_damping_y
    result_y = minimize(validate_damping, [washout_algorithm.washout_damping_y], args=('y',), bounds=[(0.000001, 0.01)])
    optimized_damping_y = result_y.x[0] if result_y.fun != np.inf else washout_algorithm.washout_damping_y

    # Optimize washout_damping_z
    result_z = minimize(validate_damping, [washout_algorithm.washout_damping_z], args=('z',), bounds=[(0.000001, 0.01)])
    optimized_damping_z = result_z.x[0] if result_z.fun != np.inf else washout_algorithm.washout_damping_z

    # Update values in the washout algorithm only if they were valid
    washout_algorithm.washout_damping_x = optimized_damping_x
    washout_algorithm.washout_damping_y = optimized_damping_y
    washout_algorithm.washout_damping_z = optimized_damping_z

    # Print optimized values
    print(f"Final Optimized washout_damping_x: {optimized_damping_x}")
    print(f"Final Optimized washout_damping_y: {optimized_damping_y}")
    print(f"Final Optimized washout_damping_z: {optimized_damping_z}")
...
if __name__ == "__main__":
    ...
    processed_movement = washout_algorithm.process(accel_data, rotational_data)
    optimize_damping(washout_algorithm, accel_data, time, Irobot)
   ...
   # Small epsilon to prevent division by zero
    ...
    # force_y_factor = np.abs(force_y_post - force_y) / (np.abs(force_y) + epsilon)
    # force_z_factor = np.abs(force_z_post - force_z) / (np.abs(force_z) + epsilon)
...
#     plt.title("Processed Angle Data")
    ...
    # plt.figure(figsize=(18, 10))
    # plt.subplot(421)
    # plt.plot(data['Time'], force_data['Force_X'], label="Raw Force X")
    # plt.plot(data['Time'], forces_after_motion_cueing['Force_X_post'], label="Force_X_post (After Motion Cueing)")
    # plt.xlabel("Time (s)")
    # plt.ylabel("Factor X")
    # plt.title("Force_X Comparison Before and After Motion Cueing")
    # plt.legend()


    # plt.subplot(422)
    # plt.plot(data['Time'], force_factor['Force_X_Factor'], label="Force factor in X")
    # plt.ylabel("Force (N)")
    # plt.title("Force_X division After Motion Cueing to raw vales")
    # plt.legend()

    # plt.subplot(423)
    # plt.plot(data['Time'], force_data['Force_Y'], label="Raw Force Y")
    # plt.plot(data['Time'], forces_after_motion_cueing['Force_Y_post'], label="Force_Y_post (After Motion Cueing)")
    # plt.xlabel("Time (s)")
    # plt.ylabel("Force (N)")
    # plt.title("Force_Y Comparison Before and After Motion Cueing")
    # plt.legend()

    # plt.subplot(424)
    # plt.plot(data['Time'], force_factor['Force_Y_Factor'], label="Force factor in Y")
    # plt.ylabel("Factor Y")
    # plt.title("Force_Y division After Motion Cueing to raw vales")

    # plt.legend()

    # plt.subplot(425)
    # plt.plot(data['Time'], force_data['Force_Z'], label="Raw Force Z")
    # plt.plot(data['Time'], forces_after_motion_cueing['Force_Z_post'], label="Force_Z_post (After Motion Cueing)")
    # plt.xlabel("Time (s)")
    # plt.ylabel("Force (N)")
    # plt.title("Force_Z Comparison Before and After Motion Cueing")
    # plt.legend()

    # plt.subplot(426)
    # plt.plot(data['Time'], force_factor['Force_Z_Factor'], label="Force factor in Z")
    # plt.ylabel("Factor Z")
    # plt.title("Force_Z division After Motion Cueing to raw vales")
    # plt.legend()
