...
class ClassicalWashout:
    ...
    def process(self, accel_data, rotational_data):
        ...
        highpass_accel_x = butter_filter(accel_data['x'], self.hp_acc_x, self.fs, order = 1, filter_type = 'high')
        highpass_accel_y = butter_filter(accel_data['y'], self.hp_acc_y, self.fs, order = 1, filter_type = 'high')
        highpass_accel_z = butter_filter(accel_data['z'], self.hp_acc_z, self.fs, order = 1, filter_type = 'high')
       ...
       # print(highpass_accel_y)
        ...
        tilt_x = (accel_data['x']/-(Acc_g))
        tilt_y = (accel_data['y']/-(Acc_g))
        ...
        highpass_roll  = butter_filter(rotational_data['roll'],  self.hp_roll,  self.fs, order =1, filter_type= 'high')
        highpass_pitch = butter_filter(rotational_data['pitch']*0.5, self.hp_pitch, self.fs, order =1, filter_type= 'high')
        highpass_yaw   = butter_filter(rotational_data['yaw'],   self.hp_yaw,   self.fs, order =1, filter_type= 'high')
