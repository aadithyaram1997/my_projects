...
def main():
   ...
   # Small epsilon to prevent division by zero
    ...
    # Calculate factors
    force_x_factor = np.abs(force_x_post - force_x) / (np.abs(force_x) + 2)
...
def optimize_washout_damping():
    ...
    def optimize_axis(axis_name):
        default = {'x': 0.005, 'y': 0.005, 'z': 0.005}
        best_cost = float('inf')
        best_value = default[axis_name]

        def cost_fn(param):
            nonlocal best_cost, best_value
            wx = param[0] if axis_name == 'x' else default['x']
            wy = param[0] if axis_name == 'y' else default['y']
            wz = param[0] if axis_name == 'z' else default['z']
            ...
            if axis_name == 'x':
                cost = np.nanmean(np.abs(force_x_post - force_x) / (np.abs(force_x) + 2))
            elif axis_name == 'y':
                cost = np.nanmean(np.abs(force_y_post - force_y) / (np.abs(force_y) + 2))
            else:
                cost = np.nanmean(np.abs(force_z_post - force_z) / (np.abs(force_z) + 2))

            if cost < best_cost:
                best_cost = cost
                best_value = param[0]
                print(f"✅ Feasible {axis_name.upper()} | Damping: {param[0]:.5f} | Cost: {cost:.4f}")

            return cost

        result = minimize(cost_fn, [0.01], bounds=[(0.0005, 0.05)], method='L-BFGS-B')
        return best_value
    ...
    print(f"Optimized washout damping X: {wx}")
    print(f"Optimized washout damping Y: {wy}")
    print(f"Optimized washout damping Z: {wz}")
