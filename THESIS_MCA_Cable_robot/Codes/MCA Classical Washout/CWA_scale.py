...
class ClassicalWashout:
    ...
    def process(self, accel_data, rotational_data):
        ...
        highpass_accel_x = butter_filter(scaled_ax_hp, self.hp_acc_x, self.fs, order = 1, filter_type = 'high')
        highpass_accel_y = butter_filter(scaled_ay_hp, self.hp_acc_y, self.fs, order = 1, filter_type = 'high')
        highpass_accel_z = butter_filter(scaled_az_hp, self.hp_acc_z, self.fs, order = 1, filter_type = 'high')

        #print(highpass_accel_y)

           # second order High-pass filter for accelerations
        highpass_accel_x_2 = butter_filter(highpass_accel_x, self.hp_acc_x, self.fs, order = 2, filter_type = 'high')
        highpass_accel_y_2 = butter_filter(highpass_accel_y, self.hp_acc_y, self.fs, order = 2, filter_type = 'high')
        highpass_accel_z_2 = butter_filter(highpass_accel_z, self.hp_acc_z, self.fs, order = 2, filter_type = 'high')
        ...
        highpass_roll  = butter_filter(scaled_roll,  self.hp_roll,  self.fs, order =1, filter_type= 'high')
        highpass_pitch = butter_filter(scaled_pitch, self.hp_pitch, self.fs, order =1, filter_type= 'high')
        highpass_yaw   = butter_filter(scaled_yaw,   self.hp_yaw,   self.fs, order =1, filter_type= 'high')

        #2nd high-pass filter for rotational rates 
        highpass_roll_2  = butter_filter(highpass_roll,  self.hp_roll,  self.fs, order =2, filter_type= 'high')
        highpass_pitch_2 = butter_filter(highpass_pitch, self.hp_pitch, self.fs, order =2, filter_type= 'high')
        highpass_yaw_2   = butter_filter(highpass_yaw,   self.hp_yaw,   self.fs, order =2, filter_type= 'high')
        ...
        roll_angle = it.cumulative_trapezoid(highpass_roll_2, dx=1 / self.fs, initial=0)
        pitch_angle = it.cumulative_trapezoid(highpass_pitch_2, dx=1 / self.fs, initial=0)
        ...
        washout_x = pos_x * np.exp(-np.linspace(0, 1, len(pos_x)))
        washout_y = pos_y * np.exp(-np.linspace(0, 1, len(pos_y)))
        washout_z = pos_z * np.exp(-np.linspace(0, 1, len(pos_z)))
        ...
        total_roll = washout_roll + tilt_y_angle_deg
        total_pitch = washout_pitch + tilt_x_angle_deg
...
if __name__ == "__main__":
    ...
    raw_x = it.cumulative_trapezoid(accel_data['x'], dx=2 / fs, initial=0)
    raw_y = it.cumulative_trapezoid(accel_data['y'], dx=2 / fs, initial=0)
    raw_z = it.cumulative_trapezoid(accel_data['z'], dx=2 / fs, initial=0)

    plt.subplot(622)
    #plt.plot(data['Time'], raw_x, label="raw_x")
    plt.plot(data['Time'], raw_y, label="raw_y")
    #plt.plot(data['Time'], raw_z, label="raw_z")
    plt.xlabel("Time (s)")
    plt.ylabel("m")
    plt.legend()
    plt.title("raw_pos")
    ...
    plt.subplot(624)
    plt.plot(data['Time'], processed_movement['lowpass_accel_x'], label="X Acceleration (High-Pass)")
    plt.plot(data['Time'], processed_movement['lowpass_accel_y'], label="Y Acceleration (High-Pass)")
    #plt.plot(data['Time'], processed_movement['lowpass_accel_z'], label="Z Acceleration (High-Pass)")
    plt.xlabel("Time (s)")
    plt.ylabel("Acceleration (m/s²)")
    plt.legend()
    plt.title("Lowpass Filtered Accelerations")
