...
class ClassicalWashout:
    ...
    def process(self, accel_data, rotational_data):
       ...
       # Integrate HP accelerations to obtain velocities and then positions
        ...
        pos_x = washout_x * np.exp(-np.linspace(0, 0, len(washout_x)))
        pos_y = washout_y * np.exp(-np.linspace(0, 0, len(washout_y)))
        pos_z = washout_z * np.exp(-np.linspace(0, 0, len(washout_z)))
        ...
        roll_angle = washout_roll * np.exp(-np.linspace(0, 0, len(washout_roll)))
        pitch_angle = washout_pitch * np.exp(-np.linspace(0, 0, len(washout_pitch)))
        yaw_angle = washout_yaw * np.exp(-np.linspace(2, 3, len(washout_yaw)))
...
if __name__ == "__main__":
    ...
    # washout_damping_b = 0.005
    # washout_damping_c = 0.005
